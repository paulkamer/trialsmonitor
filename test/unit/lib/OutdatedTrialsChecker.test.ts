...
import OutdatedTrialsChecker from '../../../src/OutdatedTrialsChecker';
...
describe('OutdatedTrialsChecker', () => {
  ...
  const trialsToCheck = Object.entries(testTrials).reduce(
    (o, [trialId, trial]) => ({ ...o, [trialId]: { lastUpdated: trial.current } }),
    {}
  );
  ...
  context('Determining outdated trials', () => {
    it('determines outdated trials', async () => {
      checker = new OutdatedTrialsChecker(db);
