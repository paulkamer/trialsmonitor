...
class UpdatesNotifier {
  ...
  constructor(db: IDbHelper, trialIds: TrialId[]) {
    this.db = db;
  ...
  async prepareEmailParameters(trials: Trial[]) {
   ...
   * @param {*} param
   */
  formatBody({ trials, lineEnd }: FormatBodyOptions) {
   ...
   */
  formatTrialLines({ trials, lineEnd }: FormatBodyOptions) {
   ...
   * @param {*} param
   */
  async sendEmail({ toAddresses, subject, textBody, htmlBody }: any) {
